        ...
        >
          <template v-slot:activator="{ on, attrs }">
            <v-btn dark color="main_bg_color" v-bind="attrs" v-on="on">
              <i class="fas fa-plus mr-2"></i>Add Company</v-btn
            >
          </template>
            ...
            <v-card>
              <v-toolbar color="main_bg_color" dark class="mb-2">
                <span v-if="isEdit == false"
                  ><i class="fas fa-stream Orders mr-2"></i>Add Company</span
                >
                <span v-else
                  ><i class="fas fa-stream Orders mr-2"></i>Edit Company</span
                >
              </v-toolbar>
              <v-card-text>
                <addCompany @closeIt="closeModel"></addCompany>
              </v-card-text>
            </v-card>
          </template>
        </v-dialog>
...
export default {
  ...
  methods: {
    ...
    editItem(obj) {
      console.log(obj);
      this.$store.dispatch("getcompanyById", obj);
      this.dialog = true;
      this.isEdit = true;
    },
    ...
    async deleteItem() {
      debugger; // eslint-disable-line no-debugger
      this.$store.dispatch("removecompany", this.itemId);
      await this.fetchData();
      this.delDialog = false;
      this.itemId = "";
